...
calculate_file_path = '房租計算表.csv'
calculate_file_path2 = '收租明細表.csv'

df = pd.read_csv('房租計算表.csv', encoding='utf-8')
df2 = pd.read_csv('收租明細表.csv', encoding='utf-8')
...
def ChooseRoom():
    ...
    if roomindex != -1:
        ...
        def ChangeRent():
            ...
            gui_change_rent.geometry('800x600')
            ...
            changebtt = ttk.Button(gui_change_rent, text="更改資料",command=ChangeRent_inside, width=7)
            changebtt.grid(column=2, row=20, ipadx=10,sticky='W')#, pady=5)
        ...
        calculate_btn.grid(column=2, row=20, ipadx=5, pady=5)

        # 確認繳費按鈕
        confirmbtt = ttk.Button(window, text="確認繳費", width=8, command=ConfirmPay)
        confirmbtt.grid(column=3, row=13, ipadx=5,ipady=35 ,rowspan=3,sticky='NW'+'SE')# ,pady=5, )
